...
void Parser::statement()
{
	...
	// станет известным только после того, как будет сгенерирован код для блока THEN.
		...
		mustBe(T_IDENTIFIER);
		int address_of_identifier = findOrAddVariable(scanner_->getStringValue());
		mustBe(T_ASSIGN);
		expression();
		codegen_->emit(STORE, address_of_identifier);

		mustBe(T_TO);
		expression();
		int  adress_of_to_value = lastVar_++;
		codegen_->emit(STORE, adress_of_to_value);

		mustBe(T_STEP);
		expression();
		int adress_of_step_value = lastVar_++;
		codegen_->emit(STORE, adress_of_step_value);


		int jmpAddress = codegen_->getCurrentAddress();

		codegen_->emit(LOAD, address_of_identifier);
		codegen_->emit(LOAD, adress_of_to_value);
		codegen_->emit(COMPARE, 1);

		int jmpNoAddress = codegen_->reserve();

		mustBe(T_DO);
		statementList();
		mustBe(T_OD);

		codegen_->emit(LOAD, adress_of_step_value);
		codegen_->emit(LOAD, address_of_identifier);
		codegen_->emit(ADD);
		codegen_->emit(STORE, address_of_identifier);

		codegen_->emit(JUMP, jmpAddress);
		codegen_->emitAt(jmpNoAddress, JUMP_NO, codegen_->getCurrentAddress());
